namespace DMEWorks.Maintain.Auxilary
{
    ...
    [DesignerGenerated]
    public class FormAuthorizationType : FormAutoIncrementMaintain
    {
        private IContainer components;
        private const string TableName = "tbl_authorizationtype";
        ...
        public FormAuthorizationType()
        {
            ...
            string[] textArray1 = new string[] { "tbl_authorizationtype" };
        ...
        protected override void DeleteObject(int ID)
        {
            using (MySqlConnection connection = Globals.CreateConnection())
            {
                ...
                using (MySqlCommand command = new MySqlCommand("", connection))
                {
                    ...
                    if (0 >= command.ExecuteDelete("tbl_authorizationtype"))
                    {
        ...
        protected override FormMaintainBase.StandardMessages GetMessages()
        {
            ...
            messages.ConfirmDeleting = $"Are you really want to delete authorization type '{this.txtName.Text}'?";
            messages.DeletedSuccessfully = $"Authorization type '{this.txtName.Text}' was successfully deleted.";
            return messages;
        }
        ...
        private void InitializeComponent()
        {
            ...
            base.Name = "FormAuthorizationType";
            this.Text = "Maintain Authorization Type";
        ...
        protected override bool LoadObject(int ID)
        {
            using (MySqlConnection connection = Globals.CreateConnection())
            {
                ...
                using (MySqlCommand command = new MySqlCommand("", connection))
                {
                    command.CommandText = "SELECT * FROM tbl_authorizationtype WHERE ID = :ID";
                    command.Parameters.Add("ID", MySqlType.Int).Value = ID;
        ...
        {
            string[] tableNames = new string[] { "tbl_authorizationtype" };
            ...
            using (MySqlConnection connection = Globals.CreateConnection())
            {
                ...
                using (MySqlCommand command = new MySqlCommand("", connection))
                {
                    ...
                    if (!IsNew)
                    {
                        ...
                        flag = 0 < command.ExecuteUpdate("tbl_authorizationtype", whereParameters);
                        flag ??= (0 < command.ExecuteInsert("tbl_authorizationtype"));
                    }
                    else
                    {
                        flag = 0 < command.ExecuteInsert("tbl_authorizationtype");
        ...
        {
            using (MySqlDataAdapter adapter = new MySqlDataAdapter("SELECT ID, Name FROM tbl_authorizationtype ORDER BY Name", ClassGlobalObjects.ConnectionString_MySql))
            {
        ...
        protected override void ValidateObject(int ID, bool IsNew)
        {
            ...
            else
            {
                ...
                if (IsNew)
                {
                    ...
                    num = Convert.ToInt32(Globals.ExecuteScalar("SELECT COUNT(*) as `Count` FROM tbl_authorizationtype WHERE (Name = :Name)", parameters));
                }
                else
                {
                    ...
                    num = Convert.ToInt32(Globals.ExecuteScalar("SELECT COUNT(*) as `Count` FROM tbl_authorizationtype WHERE (Name = :Name) AND (ID != :ID)", parameters));
                }
                if (0 < num)
                {
