namespace DMEWorks.Maintain.Auxilary
{
    ...
    [DesignerGenerated]
    public class FormShippingMethod : FormAutoIncrementMaintain
    {
        private IContainer components;
        private const string TableName = "tbl_shippingmethod";
        ...
        public FormShippingMethod()
        {
            ...
            string[] textArray1 = new string[] { "tbl_shippingmethod" };
        ...
        protected override void ClearObject()
        {
            ...
            Functions.SetComboBoxItem(this.cmbType, "None");
        }
        ...
        protected override void DeleteObject(int ID)
        {
            using (MySqlConnection connection = Globals.CreateConnection())
            {
                ...
                using (MySqlCommand command = new MySqlCommand("", connection))
                {
                    ...
                    if (0 >= command.ExecuteDelete("tbl_shippingmethod"))
                    {
        ...
        protected override FormMaintainBase.StandardMessages GetMessages()
        {
            ...
            messages.ConfirmDeleting = $"Are you really want to delete shipping method '{this.txtName.Text}'?";
            messages.DeletedSuccessfully = $"Shipping method '{this.txtName.Text}' was successfully deleted.";
            return messages;
        }

        protected override void InitDropdowns()
        {
            this.Load_Table_ShippingMethod_Type();
        }
        ...
        private void InitializeComponent()
        {
            ...
            this.cmbType.Location = new Point(80, 0x48);
            this.cmbType.Name = "cmbType";
            this.cmbType.Size = new Size(120, 0x15);
            this.cmbType.TabIndex = 5;
            this.lblType.BackColor = Color.Transparent;
            this.lblType.Location = new Point(8, 0x48);
            this.lblType.Name = "lblType";
            this.lblType.Size = new Size(0x40, 0x15);
            this.lblType.TabIndex = 4;
            this.lblType.Text = "Type";
            this.lblType.TextAlign = ContentAlignment.MiddleRight;
            this.AutoScaleBaseSize = new Size(5, 13);
            base.ClientSize = new Size(0x228, 0x189);
            base.Name = "FormShippingMethod";
            this.Text = "Form Shipping Method";
        ...
        private void Load_Table_ShippingMethod_Type()
        {
            this.cmbType.BeginUpdate();
            try
            {
                ...
                try
                {
                    enumerator = Enum.GetValues(typeof(ShippingTypeEnum)).GetEnumerator();
                    while (enumerator.MoveNext())
                    {
                        ShippingTypeEnum enum2 = (ShippingTypeEnum) Conversions.ToByte(enumerator.Current);
                        this.cmbType.Items.Add(ShippingType.ToString(enum2));
                    }
                }
                finally
                {
        ...
        protected override bool LoadObject(int ID)
        {
            using (MySqlConnection connection = Globals.CreateConnection())
            {
                ...
                using (MySqlCommand command = new MySqlCommand("", connection))
                {
                    command.CommandText = "SELECT * FROM tbl_shippingmethod WHERE ID = :ID";
                    command.Parameters.Add("ID", MySqlType.Int).Value = ID;
                    using (MySqlDataReader reader = command.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            ShippingTypeEnum enum2 = ShippingType.ToEnum(reader["Type"] as string);
                            Functions.SetComboBoxItem(this.cmbType, ShippingType.ToString(enum2));
                        }
                        else
                        {
                            return false;
        ...
        {
            string[] tableNames = new string[] { "tbl_shippingmethod" };
        ...
        protected override bool SaveObject(int ID, bool IsNew)
        {
            ...
            using (MySqlConnection connection = Globals.CreateConnection())
            {
                ...
                using (MySqlCommand command = new MySqlCommand("", connection))
                {
                    ...
                    command.Parameters.Add("Type", MySqlType.VarChar, 50).Value = this.cmbType.SelectedItem;
                    command.Parameters.Add("LastUpdateUserID", MySqlType.SmallInt).Value = Globals.CompanyUserID;
                    if (!IsNew)
                    {
                        command.Parameters.Add("ID", MySqlType.Int).Value = ID;
                        string[] whereParameters = new string[] { "ID" };
                        flag = 0 < command.ExecuteUpdate("tbl_shippingmethod", whereParameters);
                        flag ??= (0 < command.ExecuteInsert("tbl_shippingmethod"));
                    }
                    else
                    {
                        flag = 0 < command.ExecuteInsert("tbl_shippingmethod");
        ...
        {
            using (MySqlDataAdapter adapter = new MySqlDataAdapter("SELECT ID, Name FROM tbl_shippingmethod ORDER BY Name", ClassGlobalObjects.ConnectionString_MySql))
            {
        ...
        {
            _Closure$__24-0 e$__- = new _Closure$__24-0 {
                $VB$Local_args = args
            };
        ...
        protected override void ValidateObject(int ID, bool IsNew)
        {
            ...
            else
            {
                ...
                if (IsNew)
                {
                    ...
                    num = Convert.ToInt32(Globals.ExecuteScalar("SELECT COUNT(*) as `Count` FROM tbl_shippingmethod WHERE (Name = :Name)", parameters));
                }
                else
                {
                    ...
                    num = Convert.ToInt32(Globals.ExecuteScalar("SELECT COUNT(*) as `Count` FROM tbl_shippingmethod WHERE (Name = :Name) AND (ID != :ID)", parameters));
                }
                if (0 < num)
                {
