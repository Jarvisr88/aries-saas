namespace DMEWorks.Maintain.Auxilary
{
    ...
    [DesignerGenerated]
    public class FormPriceCode : FormAutoIncrementMaintain
    {
        private IContainer components;
        private const string TableName = "tbl_pricecode";

        public FormPriceCode()
        {
            ...
            string[] textArray1 = new string[] { "tbl_pricecode" };
        ...
        protected override void DeleteObject(int ID)
        {
            using (MySqlConnection connection = Globals.CreateConnection())
            {
                ...
                using (MySqlCommand command = new MySqlCommand("", connection))
                {
                    ...
                    if (0 >= command.ExecuteDelete("tbl_pricecode"))
                    {
        ...
        protected override FormMaintainBase.StandardMessages GetMessages()
        {
            ...
            messages.ConfirmDeleting = $"Are you really want to delete price code '{this.txtName.Text}'?";
            messages.DeletedSuccessfully = $"Price code '{this.txtName.Text}' was successfully deleted.";
            return messages;
        }
        ...
        private void InitializeComponent()
        {
            ...
            base.ToolTip1.SetToolTip(this.txtName, "If price code name contains RETAIL it will be considered as retail price level");
            this.AutoScaleBaseSize = new Size(5, 13);
            base.ClientSize = new Size(0x228, 0x189);
            base.Name = "FormPriceCode";
            this.Text = "Form Price Code";
        ...
        protected override bool LoadObject(int ID)
        {
            using (MySqlConnection connection = Globals.CreateConnection())
            {
                ...
                using (MySqlCommand command = new MySqlCommand("", connection))
                {
                    command.CommandText = "SELECT * FROM tbl_pricecode WHERE ID = :ID";
                    command.Parameters.Add("ID", MySqlType.Int).Value = ID;
        ...
        {
            string[] tableNames = new string[] { "tbl_pricecode" };
            ...
            using (MySqlConnection connection = Globals.CreateConnection())
            {
                ...
                using (MySqlCommand command = new MySqlCommand("", connection))
                {
                    ...
                    if (!IsNew)
                    {
                        ...
                        flag = 0 < command.ExecuteUpdate("tbl_pricecode", whereParameters);
                        flag ??= (0 < command.ExecuteInsert("tbl_pricecode"));
                    }
                    else
                    {
                        flag = 0 < command.ExecuteInsert("tbl_pricecode");
        ...
        {
            using (MySqlDataAdapter adapter = new MySqlDataAdapter("SELECT ID, Name FROM tbl_pricecode ORDER BY Name", ClassGlobalObjects.ConnectionString_MySql))
            {
        ...
        protected override void ValidateObject(int ID, bool IsNew)
        {
            ...
            else
            {
                ...
                if (IsNew)
                {
                    ...
                    num = Convert.ToInt32(Globals.ExecuteScalar("SELECT COUNT(*) as `Count` FROM tbl_pricecode WHERE (Name = :Name)", parameters));
                }
                else
                {
                    ...
                    num = Convert.ToInt32(Globals.ExecuteScalar("SELECT COUNT(*) as `Count` FROM tbl_pricecode WHERE (Name = :Name) AND (ID != :ID)", parameters));
                }
                if (0 < num)
                {
