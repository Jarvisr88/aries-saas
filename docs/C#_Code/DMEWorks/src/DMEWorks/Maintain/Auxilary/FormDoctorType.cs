namespace DMEWorks.Maintain.Auxilary
{
    ...
    [DesignerGenerated]
    public class FormDoctorType : FormAutoIncrementMaintain
    {
        private IContainer components;
        private const string TableName = "tbl_doctortype";

        public FormDoctorType()
        {
            ...
            string[] textArray1 = new string[] { "tbl_doctortype" };
        ...
        protected override void DeleteObject(int ID)
        {
            using (MySqlConnection connection = Globals.CreateConnection())
            {
                ...
                using (MySqlCommand command = new MySqlCommand("", connection))
                {
                    ...
                    if (0 >= command.ExecuteDelete("tbl_doctortype"))
                    {
        ...
        protected override FormMaintainBase.StandardMessages GetMessages()
        {
            ...
            messages.ConfirmDeleting = $"Are you really want to delete doctor type '{this.txtName.Text}'?";
            messages.DeletedSuccessfully = $"Doctor type '{this.txtName.Text}' was successfully deleted.";
            return messages;
        }
        ...
        private void InitializeComponent()
        {
            ...
            base.Name = "FormDoctorType";
            this.Text = "Form Doctor Type";
        ...
        protected override bool LoadObject(int ID)
        {
            using (MySqlConnection connection = Globals.CreateConnection())
            {
                ...
                using (MySqlCommand command = new MySqlCommand("", connection))
                {
                    command.CommandText = "SELECT * FROM tbl_doctortype WHERE ID = :ID";
                    command.Parameters.Add("ID", MySqlType.Int).Value = ID;
        ...
        {
            string[] tableNames = new string[] { "tbl_doctortype" };
            ...
            using (MySqlConnection connection = Globals.CreateConnection())
            {
                ...
                using (MySqlCommand command = new MySqlCommand("", connection))
                {
                    ...
                    if (!IsNew)
                    {
                        ...
                        flag = 0 < command.ExecuteUpdate("tbl_doctortype", whereParameters);
                        flag ??= (0 < command.ExecuteInsert("tbl_doctortype"));
                    }
                    else
                    {
                        flag = 0 < command.ExecuteInsert("tbl_doctortype");
        ...
        {
            using (MySqlDataAdapter adapter = new MySqlDataAdapter("SELECT ID, Name FROM tbl_doctortype ORDER BY Name", ClassGlobalObjects.ConnectionString_MySql))
            {
        ...
        protected override void ValidateObject(int ID, bool IsNew)
        {
            ...
            else
            {
                ...
                if (IsNew)
                {
                    ...
                    num = Convert.ToInt32(Globals.ExecuteScalar("SELECT COUNT(*) as `Count` FROM tbl_doctortype WHERE (Name = :Name)", parameters));
                }
                else
                {
                    ...
                    num = Convert.ToInt32(Globals.ExecuteScalar("SELECT COUNT(*) as `Count` FROM tbl_doctortype WHERE (Name = :Name) AND (ID != :ID)", parameters));
                }
                if (0 < num)
                {
