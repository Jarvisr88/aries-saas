namespace DMEWorks.Maintain.Auxilary
{
    ...
    [DesignerGenerated]
    public class FormProductType : FormAutoIncrementMaintain
    {
        private IContainer components;
        private const string TableName = "tbl_producttype";
        ...
        public FormProductType()
        {
            ...
            string[] textArray1 = new string[] { "tbl_producttype" };
        ...
        protected override void DeleteObject(int ID)
        {
            using (MySqlConnection connection = Globals.CreateConnection())
            {
                ...
                using (MySqlCommand command = new MySqlCommand("", connection))
                {
                    ...
                    if (0 >= command.ExecuteDelete("tbl_producttype"))
                    {
        ...
        protected override FormMaintainBase.StandardMessages GetMessages()
        {
            ...
            messages.ConfirmDeleting = $"Are you really want to delete product type '{this.txtName.Text}'?";
            messages.DeletedSuccessfully = $"Product type '{this.txtName.Text}' was successfully deleted.";
            return messages;
        }
        ...
        private void InitializeComponent()
        {
            ...
            base.Name = "FormProductType";
            this.Text = "Form Product Type";
        ...
        protected override bool LoadObject(int ID)
        {
            using (MySqlConnection connection = Globals.CreateConnection())
            {
                ...
                using (MySqlCommand command = new MySqlCommand("", connection))
                {
                    command.CommandText = "SELECT * FROM tbl_producttype WHERE ID = :ID";
                    command.Parameters.Add("ID", MySqlType.Int).Value = ID;
        ...
        {
            string[] tableNames = new string[] { "tbl_producttype" };
            ...
            using (MySqlConnection connection = Globals.CreateConnection())
            {
                ...
                using (MySqlCommand command = new MySqlCommand("", connection))
                {
                    ...
                    if (!IsNew)
                    {
                        ...
                        flag = 0 < command.ExecuteUpdate("tbl_producttype", whereParameters);
                        flag ??= (0 < command.ExecuteInsert("tbl_producttype"));
                    }
                    else
                    {
                        flag = 0 < command.ExecuteInsert("tbl_producttype");
        ...
        {
            using (MySqlDataAdapter adapter = new MySqlDataAdapter("SELECT ID, Name FROM tbl_producttype ORDER BY Name", ClassGlobalObjects.ConnectionString_MySql))
            {
        ...
        protected override void ValidateObject(int ID, bool IsNew)
        {
            ...
            else
            {
                ...
                if (IsNew)
                {
                    ...
                    num = Convert.ToInt32(Globals.ExecuteScalar("SELECT COUNT(*) as `Count` FROM tbl_producttype WHERE (Name = :Name)", parameters));
                }
                else
                {
                    ...
                    num = Convert.ToInt32(Globals.ExecuteScalar("SELECT COUNT(*) as `Count` FROM tbl_producttype WHERE (Name = :Name) AND (ID != :ID)", parameters));
                }
                if (0 < num)
                {
