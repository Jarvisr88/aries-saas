namespace DMEWorks.Maintain.Auxilary
{
    ...
    [DesignerGenerated]
    public class FormReferralType : FormAutoIncrementMaintain
    {
        private IContainer components;
        private const string TableName = "tbl_referraltype";

        public FormReferralType()
        {
            ...
            string[] textArray1 = new string[] { "tbl_referraltype" };
        ...
        protected override void DeleteObject(int ID)
        {
            using (MySqlConnection connection = Globals.CreateConnection())
            {
                ...
                using (MySqlCommand command = new MySqlCommand("", connection))
                {
                    ...
                    if (0 >= command.ExecuteDelete("tbl_referraltype"))
                    {
        ...
        protected override FormMaintainBase.StandardMessages GetMessages()
        {
            ...
            messages.ConfirmDeleting = $"Are you really want to delete referral type '{this.txtName.Text}'?";
            messages.DeletedSuccessfully = $"Referral type '{this.txtName.Text}' was successfully deleted.";
            return messages;
        }
        ...
        private void InitializeComponent()
        {
            ...
            this.txtName.Size = new Size(0x198, 0x15);
            this.txtName.TabIndex = 3;
            this.AutoScaleBaseSize = new Size(5, 13);
            base.ClientSize = new Size(0x228, 0x189);
            base.Name = "FormReferralType";
            this.Text = "Form Referral Type";
        ...
        protected override bool LoadObject(int ID)
        {
            using (MySqlConnection connection = Globals.CreateConnection())
            {
                ...
                using (MySqlCommand command = new MySqlCommand("", connection))
                {
                    command.CommandText = "SELECT * FROM tbl_referraltype WHERE ID = :ID";
                    command.Parameters.Add("ID", MySqlType.Int).Value = ID;
        ...
        {
            string[] tableNames = new string[] { "tbl_referraltype" };
            ...
            using (MySqlConnection connection = Globals.CreateConnection())
            {
                ...
                using (MySqlCommand command = new MySqlCommand("", connection))
                {
                    ...
                    if (!IsNew)
                    {
                        ...
                        flag = 0 < command.ExecuteUpdate("tbl_referraltype", whereParameters);
                        flag ??= (0 < command.ExecuteInsert("tbl_referraltype"));
                    }
                    else
                    {
                        flag = 0 < command.ExecuteInsert("tbl_referraltype");
        ...
        {
            using (MySqlDataAdapter adapter = new MySqlDataAdapter("SELECT ID, Name FROM tbl_referraltype ORDER BY Name", ClassGlobalObjects.ConnectionString_MySql))
            {
        ...
        protected override void ValidateObject(int ID, bool IsNew)
        {
            ...
            else
            {
                ...
                if (IsNew)
                {
                    ...
                    num = Convert.ToInt32(Globals.ExecuteScalar("SELECT COUNT(*) as `Count` FROM tbl_referraltype WHERE (Name = :Name)", parameters));
                }
                else
                {
                    ...
                    num = Convert.ToInt32(Globals.ExecuteScalar("SELECT COUNT(*) as `Count` FROM tbl_referraltype WHERE (Name = :Name) AND (ID != :ID)", parameters));
                }
                if (0 < num)
                {
