namespace DevExpress.XtraExport.Helpers
{
    ...
    public class BandedGridViewExcelExporter<TCol, TRow> : GridViewExcelExporter<TCol, TRow> where TCol: class, IColumn where TRow: class, IRowBase
    {
        public BandedGridViewExcelExporter(IBandedGridView<TCol, TRow> viewToExport) : base(viewToExport, DataAwareExportOptionsFactory.Create(ExportTarget.Xlsx))
        {
        }

        public BandedGridViewExcelExporter(IBandedGridView<TCol, TRow> viewToExport, IDataAwareExportOptions options) : base(viewToExport, options)
        {
        }

        protected override DataAwareExportContext<TCol, TRow> CreateContext(ExportInfo<TCol, TRow> exportInfo) => 
            new DataAwareBandedExportContext<TCol, TRow>(exportInfo);

        internal override ExportInfo<TCol, TRow> CreateExportInfo() => 
            new BandedExportInfo<TCol, TRow>(this);
